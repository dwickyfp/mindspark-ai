...
export async function saveMcpClientAction(
  ...
  ownerUserId: string,
  options?: {
    shareWithOrganizationIds?: string[];
  },
) {
  ...
  const persisted = await mcpClientsManager.persistClient({
    ...server,
    ownerUserId,
  });

  if (options?.shareWithOrganizationIds?.length) {
    const serverId = server.id ?? persisted.id;
    await Promise.all(
      options.shareWithOrganizationIds.map((organizationId) =>
        organizationRepository.addSharedMcpServer(organizationId, serverId),
      ),
    );
  }
